...
export default function EditorPage() {
  ...
  useEffect(() => {
    const verifyPaymentAndLoadDocument = async () => {
      try {
        ...
        // Send email backup after payment (if session_id present and email available)
        if (sessionId) {
          sendEmailBackup(parsed, docId);
        }
  ...
  // Send email backup after successful payment
  const sendEmailBackup = async (doc: GeneratedDocument, docId: string) => {
    try {
      // Get email from localStorage (entered on preview page or contact details)
      ...
      if (formData) {
        try {
          ...
          userEmail = parsed?.contactDetails?.email || '';
        } catch (error) {
          logger.warn('Could not parse form data for email', { error });
        }
      }

      // Also check if email was stored from preview page
      const previewEmail = localStorage.getItem(`email-${docId}`);
      if (previewEmail) {
        userEmail = previewEmail;
      }

      if (!userEmail || !userEmail.includes('@')) {
        logger.info('No email provided, skipping email backup', { documentId: docId });
        return;
      }

      setEmailSending(true);
      logger.info('Sending email backup', { email: userEmail, documentId: docId });
      ...
      if (result.success) {
        setEmailSent(true);
        logger.info('Email backup sent successfully', {
          email: userEmail,
          emailId: result.data?.emailId,
        });
      } else {
        logger.warn('Failed to send email backup', {
          email: userEmail,
          error: result.error,
        });
      }
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      logger.error('Email backup error', {
        documentId: docId,
        error: errorMessage,
      });
    } finally {
          ...
          <div className="mb-6 bg-accent-purple/10 border border-accent-purple/30 rounded-lg p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                {emailSending && (
                  <p className="text-xs text-accent-purple mt-1 flex items-center gap-1">
                    <svg className="w-3 h-3 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                    </svg>
                    Sending email backup...
                  </p>
                )}
                {emailSent && (
                  <p className="text-xs text-accent-purple mt-1 flex items-center gap-1">
